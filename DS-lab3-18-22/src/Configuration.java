...
public class Configuration {
	...
	private ArrayList<Client_Info> allClients;
	...
	public Configuration(String fileName) {
		...
		if (file.exists()) {
			allClients = new ArrayList<Client_Info>();
			// reading the file
			Scanner sc = null;
			try {
				sc = new Scanner(file);
	...
	private void fill(int count, Scanner sc, String type, int base) {
		for (int i = 0; i < count; i++)
			allClients.add(new Client_Info(sc.nextLine().trim().split("=")[1]
					.trim(), type, base + i));
	}
	...
	public ArrayList<Client_Info> getClients() {
